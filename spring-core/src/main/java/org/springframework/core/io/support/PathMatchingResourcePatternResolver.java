...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	/**
	 * 直接委托给内置的resourceLoader处理，默认是 {@link DefaultResourceLoader}
	 ...
	 * - 其他情况，调用 {@link #findAllClassPathResources(String)}或 {@link #findPathMatchingResources(String)}方法，返回多个 Resource 。
	 *
	...
	/**
	 * 当 locationPattern 以 "classpath*:" 开头但是不包含通配符时调用，加载资源
	...
	/**
	 * 利用 ClassLoader 来加载指定路径下的资源，
	 * 不论它是在 class 路径下还是在 jar 包中。
	 ...
	 * 不管 locationPattern 是否以 classpath*: 开头。
	 *
	...
	protected String determineRootDir(String location) {
		// 示例：classpath*:test/cc*/spring-*.xml -> classpath*:test/
